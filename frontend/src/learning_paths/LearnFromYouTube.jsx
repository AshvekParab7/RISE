...
import { BookOpen, Check, ChevronRight, CirclePlay, LockKeyhole, RotateCcw, Sparkles, Trophy, Zap } from 'lucide-react'
...
function LearnFromYouTubeContent() {
  ...
  const createPath = async event => { event.preventDefault(); setBusy(true); setError(''); try { const created = await learningPathsApi.create(url); setPath(created); setLevel(null); await loadPaths() } catch (reason) { setError(reason.message) } finally { setBusy(false) } }
  ...
  const submitCheckpoint = async () => { if (!answer.trim()) return; setBusy(true); setError(''); try { const response = await learningPathsApi.checkpoint(path.id, level.id, answer); setPath(response.path); setLevel(response.level); setFeedback(response.attempt); setAnswer(''); if (response.attempt.correct) setStep(3) } catch (reason) { setError(reason.message) } finally { setBusy(false) } }
  ...
  if (!path) return <div className="yt-learn-page"><header className="yt-hero"><div><p className="eyebrow">RISE LEARNING PATHS</p><h1>Learn from YouTube</h1><p>Turn a captioned lecture into a step-by-step learning journey.</p></div><CirclePlay size={42}/></header>{error && <p className="api-error">{error}</p>}<div className="yt-home-grid"><form className="panel yt-url-card" onSubmit={createPath}><div className="yt-url-icon"><CirclePlay size={28}/></div><h2>Paste a YouTube lecture</h2><p className="muted">RISE uses available captions. The video stays on YouTube as a reference.</p><input type="url" value={url} onChange={event => setUrl(event.target.value)} placeholder="https://www.youtube.com/watch?v=..." required/><button className="button button-primary" disabled={busy}>{busy ? 'Building learning path...' : 'Start learning'}<ChevronRight size={16}/></button></form><aside className="panel yt-continue"><h3>Continue learning</h3>{paths.length ? paths.map(item => <button onClick={() => resume(item.id)} key={item.id}><span><BookOpen size={16}/></span><div><b>{item.title || 'Processing lecture'}</b><small>{item.completed_levels}/{item.total_levels} levels · {item.xp} XP</small><i><em style={{width:`${item.total_levels ? asPercent(item.completed_levels, item.total_levels) : item.processing_progress}%`}}/></i></div><ChevronRight size={16}/></button>) : <p className="muted">Your YouTube learning paths will stay here.</p>}</aside></div></div>
  if (path.status === 'PROCESSING') return <div className="yt-processing"><div className="yt-processing-ring"><CirclePlay size={28}/></div><p className="eyebrow">BUILDING YOUR PATH</p><h1>{path.processing_stage}</h1><div className="yt-progress"><span style={{width:`${path.processing_progress}%`}}/></div><strong>{path.processing_progress}%</strong><p>RISE is turning the real transcript into bounded lessons.</p></div>
