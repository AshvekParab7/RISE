...
const friendlyMessage = (status) =>
  ({
    ...
    404: "That RISE record could not be found.",
    409: "This record already exists.",
    429: "RISE is busy right now. Please try again shortly.",
    500: "RISE had trouble processing that request.",
  })[status] || "RISE could not complete that request.";
const validationMessage = (data) => {
  if (typeof data === "string") return data;
  if (Array.isArray(data))
    return data.map(validationMessage).filter(Boolean).join(" ");
...
export async function request(path, options = {}, retried = false) {
  ...
  const hadSession = Boolean(
    token || localStorage.getItem("rise_refresh_token"),
  );
  ...
  try {
    ...
    throw new ApiError(
      0,
      "RISE can't reach the server right now. Please try again.",
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      response.status,
      validationMessage(data) || friendlyMessage(response.status),
      data,
    );
  }
...
export const hasSession = () =>
  Boolean(
    localStorage.getItem("rise_access_token") ||
    localStorage.getItem("rise_refresh_token"),
  );
