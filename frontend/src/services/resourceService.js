...
const download = async id => {
	const token = localStorage.getItem('rise_access_token')
	const response = await fetch(`${api.url}/resources/${id}/download/`, {
		headers: token ? { Authorization: `Bearer ${token}` } : {},
	})
...
export const resourceService = { list: () => get('/resources/'), upload: formData => post('/resources/', formData), remove: id => remove(`/resources/${id}/`), preview, download }
