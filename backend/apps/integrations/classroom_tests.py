...
class ClassroomSyncTests(TestCase):
    ...
    @patch('apps.integrations.services.google_classroom.MediaIoBaseDownload')
    @patch('apps.integrations.services.google_classroom.credentials_for')
    def test_drive_document_is_exported_as_pdf(self, credentials_for_mock, downloader_mock):
        credentials_for_mock.return_value = Mock()
        ...
        class DriveFiles:
            def get(self, **_kwargs): return Request({'name': 'Unit 4', 'mimeType': 'application/vnd.google-apps.document'})
            def export_media(self, **_kwargs): return Request(None)

        class DriveClient:
            def files(self): return DriveFiles()

        def build_client(api_name, *_args, **_kwargs):
            return DriveClient() if api_name == 'drive' else Mock()

        class Downloader:
            def __init__(self, buffer, _request): self.buffer = buffer
            def next_chunk(self):
                self.buffer.write(b'%PDF-classroom-content')
                return None, True

        downloader_mock.side_effect = Downloader
        service = GoogleClassroomService(self.connection, client_builder=build_client)
        result = service.download_material({'materials': [{'driveFile': {'driveFile': {'id': 'drive-file-1'}}}]})
        self.assertEqual(result['filename'], 'Unit 4.pdf')
